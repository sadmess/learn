...
#define PRIME_NUM 99
  ...
  6477510814055453699ull, 9716266221083181299ull, 14574399331624771603ull, 18446744073709551557ull
	...
	pair<typename hashtable<T, Hash, KeyEqual>::iterator, bool> hashtable<T, Hash, KeyEqual>::emplace_unique(Args&& ...args)
	{
		...
		try
		{
			if ((float)(size_ + 1) > (float)bucket_size_* get_load_factor())  //超过承载上限
				rehash(size_ + 1);
		}
		catch (...)
		{
		...
		erase(const_iterator first, const_iterator last)
	{
		...
		{
			erase_bucket(first_bucket, first.node, nullptr);//删除first_bucket的first.node起到尾
			for (auto n = first_bucket + 1; n < last_bucket; ++n) //删除first_bucket+1到last_bucket之前所有节点
			{
			...
			{
				erase_bucket(last_bucket, last.node);  //删除last_bucket到last.node的节点
			}
		}
	}
		...
		erase_unique(const key_type& key)
	{
		...
		if (first)
		{
			if (is_equal(value_traits::get_key(first->value), key)) //看看第一个是不是
			{
	...
	template <class T, class Hash, class KeyEqual>
	void hashtable<T, Hash, KeyEqual>::
		clear()  //size_变为0，但不代表筒变成零
	{
		if (size_ != 0)
		{
			for (size_type i = 0; i < bucket_size_; ++i)  //遍历桶
			{
				node_ptr cur = buckets_[i];
				while (cur != nullptr)//把桶内一个一个删除
				{
					node_ptr next = cur->next;
					destroy_node(cur);
					cur = next;
				}
				buckets_[i] = nullptr;//桶空
			}
			size_ = 0;
		}
	}
		...
		rehash(size_type count)
	{
		...
		if (n > bucket_size_)   //如果重新排的大小比现有的大，说明超过载荷确实需要重排
		{
			replace_bucket(n);
		}
		else
		{
			if ((float)size_ / (float)n < get_load_factor() - 0.25f &&
				(float)n < (float)bucket_size_ * 0.75)  // 如果n小于正常载荷75%，可以进行重排（这里规则很玄学
			{
				replace_bucket(n);
			}
		}
